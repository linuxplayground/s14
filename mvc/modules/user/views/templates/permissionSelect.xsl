...
<xsl:template match="/">
	...
	<html>
		...
		<body>
			<h2>Super 14 - Permissions for  - <xsl:value-of select="page/group_name" /></h2>
			<hr />
					...
					<td valign='top'>
						<form>
							<xsl:attribute name="action">
								<xsl:text>index.php?action=updateGroupPermission&amp;module=user&amp;id=</xsl:text>
								<xsl:value-of select="page/group_id" />
							</xsl:attribute>
							<xsl:attribute name="method">POST</xsl:attribute>
							<table>
								<xsl:apply-templates select="page/permission" />
								<tr>
...
<xsl:template match="permission">
	...
	<tr><td>
		<xsl:if test="checked='Y'">
			<input>
				<xsl:attribute name="name"><xsl:text>permission[]</xsl:text></xsl:attribute>
