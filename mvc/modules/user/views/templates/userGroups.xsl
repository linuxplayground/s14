...
<xsl:template match="/">
	...
	<html>
		...
		<body>
			<h2>Super 14 - Groups - <xsl:value-of select="page/user_name" /></h2>
			<hr />
					...
					<td valign='top'>
						<form>
							<xsl:attribute name="action">
								<xsl:text>index.php?action=updateUserGroup&amp;module=user&amp;id=</xsl:text>
								<xsl:value-of select="page/user_id" />
							</xsl:attribute>
							<xsl:attribute name="method">POST</xsl:attribute>
							<table>
								<xsl:apply-templates select="page/group" />
								<tr>
