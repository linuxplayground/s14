...
<xsl:template match="/">
	...
	<html>
		<head>
			...
			<![CDATA[
				gamesCount = ]]><xsl:value-of select = 'count(picksTable/matchTypePicks/game)' /><![CDATA[;
				loopCount = 0;
				function randomPick(roundNum) {
					tileName = "picks["+roundNum+"]";
					var tile = document.getElementById(tileName);
					var games = tile.getElementsByTagName('tr');
					for (lp = 0; lp < games.length; lp ++ ) {
						//iterate through the games
						if (games[lp].id == "game" ) {
							var margins = games[lp].getElementsByTagName('input');
							seed = Math.floor( Math.random( ) * margins.length );
							margins[seed].checked = true;
						}
					}
					loopCount++;
					if (loopCount < 40) {
						var t = setTimeout("randomPick("+roundNum+")", 50);
					} else {
						loopCount = 0;
					}
				}
				...
				function confirmPaid() {
					var paid = confirm("Click OK if you have paid your $2.00 stake for this week?");
					if ( paid == false ) {
						window.location.href = "index.php?action=pleasePay&module=help";
						return false;
					} else {
						document.getElementById('pmtConfirmed').innerHTML = "Payment: Confirmed.";
						document.pickForm.paidStake.value = 1;
						return true;
					}
				}
				...
				{
				    if (v.length != 0) {
				      location.href = 'index.php?action=getPicksForUser&module=pick&round=' + v ;
				    }
				}
				function prev_menu_goto( v )
				{
				    if (v.length != 0) {
				      location.href = 'index.php?action=getPrevPicks&module=pick&round=' + v ;
				    }
				}
				
				function delete_goto( v )
				{
				    if (v.length != 0) {
				      location.href = 'index.php?action=confirmDeletePicks&module=pick&round=' + v ;
				    }
				}
			]]>
			</script>
		...
		<body>
			<h2>Super 14 - Picks</h2>
			<hr />
			<div id='pmtConfirmed' name='pmtConfirmed' class='message' />
...
<xsl:template match="matchTypePicks">
	<form action='index.php?action=submitPicks&amp;module=pick' method='post' id='pickForm' name='pickForm'>
		<input type='hidden' name='paidStake' value='0'/>
		...
		<input>
			...
			<xsl:attribute name='name'>
				<xsl:text>selectPicksSubmit</xsl:text>
			</xsl:attribute>
			<xsl:attribute name='id'>
				<xsl:text>selectPicksSubmit</xsl:text>
			</xsl:attribute>
			<xsl:attribute name='value'>
				<xsl:text>Submit Picks</xsl:text>
			</xsl:attribute>
		...
		<button>
			<xsl:attribute name='onClick'>
				<![CDATA[randomPick(]]><xsl:value-of select='roundNumber'/><![CDATA[); return false;]]>
			</xsl:attribute>
			<xsl:text>Random Pick</xsl:text>
		</button>
		Select Round: 
		...
		<select name='roundSelect' id='roundSelect'>
			<xsl:attribute name='onchange'>
				<![CDATA[prev_menu_goto( this.options[this.selectedIndex].value );return false;]]>
			</xsl:attribute>
			<option value='all'>Select</option>
			<xsl:apply-templates select='/picksTable/prevRound'>
		...
		<!-- button>
			<xsl:attribute name='onClick'>
				<![CDATA[delete_goto(]]><xsl:value-of select='roundNumber'/><![CDATA[); return false;]]>
			</xsl:attribute>
			<xsl:text>Delete Pick</xsl:text>
		</button -->
	</form>
</xsl:template>
...
<xsl:template match='picksTable/prevRound'>
	<option>
		<xsl:attribute name='value'>
			<xsl:value-of select='.'/>
		</xsl:attribute>
